...
export class Model {
  ...
  >([intrinsic, derived, extension]: [KindShape<Kind> & Intrinsic, Derived, E]) => {
    ...
    } as Intrinsic & Optional<Derived> & Optional<E> & { kind: never }

    const derivedShape = {
      ...intrinsic,
      ...extension,
      ...derived,
    } as KindShape<Kind> & Intrinsic & E & Derived

    return [z.object(intrinsicShape).omit({ kind: true }), z.object(derivedShape)] as const
  }
}
